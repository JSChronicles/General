    ...
    param (
        ...
        [ValidateScript( {
                if (-Not ($PSItem | Test-Path) ) {
                    throw "$PSItem Path does not exist."
                }
                if ($PSItem -match "(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/))") {
                    throw "$PSItem The path specified in the argument must be a network share, file, or folder. URLs are not allowed."
                }
                return $true
            })]
        [string[]]$Path,
