function Resolve-ReagentC {
    ...
    [CmdletBinding()]
    param(
        $Partitions = (Get-Partition -DiskNumber 0 | Where-Object {$PSItem.type -match "Recovery"})
    )
    begin {
    ...
    process {
        if ($analyzeReagentcEnabled) {
            ...
            Write-Output "Windows RE Status: Enabled"
        }
        elseif ($analyzeReagentcDisabled) {
            try {
                ...
                [void](Invoke-Expression "$env:SystemDirectory\ReagentC.exe /enable")
            }
            catch {
                $logger.Error("$PSitem")
            ...
            Write-Output "Unknown Windows RE Status"
        }
                ...
                if(!([string]::IsNullOrWhiteSpace($recoveryPartition))){
                    if($recoveryPartition -match '(partition+\d)') {
                        ...
                        if($matches[0] -match'(\d)') {
                            $Partitions | Where-Object {$PSItem.PartitionNumber -notcontains "$($matches[0])"} | Remove-Partition
